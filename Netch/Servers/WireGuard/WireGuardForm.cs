...
public class WireGuardForm : ServerForm
{
    public WireGuardForm(WireGuardServer? server = default)
    {
        ...
        CreateTextBox("MTU",            "MTU",              s => int.TryParse(s, out _),    s => server.MTU = int.Parse(s),     server.MTU.ToString(), 
        CreateTextBox("Reserved",       "Reserved",         s => CheckReserved(s),          s => server.Reserved = s,           server.Reserved, 76);
        CreateTextBox("Workers",        "Workers",          s => int.TryParse(s, out _),    s => server.Workers = int.Parse(s), server.Workers.ToString
        CreateTextBox("KeepAlive",      "Keep Alive",       s => int.TryParse(s, out _),    s => server.KeepAlive = int.Parse(s),server.KeepAlive.ToStr
